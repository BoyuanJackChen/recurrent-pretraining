...
def recover_prompt(wordgame_prompt, keyword):
    ### Extract the string with [MASK]
    start = ") in the format: Here is how to "
    end = ": Step 1: <main idea of the first instruction>"
    prompt = wordgame_prompt[wordgame_prompt.find(start)+len(start):wordgame_prompt.rfind(end)]
    ### Replace [MASK] with keyword
    prompt = prompt.replace("[MASK]", keyword)
...
def main():
    ...
    keyword = "wordgame"
    all_r = [4, 8, 16, 32, 64]
    df_4 = pd.read_csv(f"../../outputs/{keyword}_harmbench/{keyword}_r4.csv")
    df_8 = pd.read_csv(f"../../outputs/{keyword}_harmbench/{keyword}_r8.csv")
    df_16 = pd.read_csv(f"../../outputs/{keyword}_harmbench/{keyword}_r16.csv")
    df_32 = pd.read_csv(f"../../outputs/{keyword}_harmbench/{keyword}_r32.csv")
    df_64 = pd.read_csv(f"../../outputs/{keyword}_harmbench/{keyword}_r64.csv")

    ### mkdir if it doesnt exist
    output_dir = f"../../outputs/{keyword}_harmbench/jailbrokens"
    ...
    for i, (row_4, row_8, row_16, row_32, row_64) in enumerate(zip(df_4.iterrows(), df_8.iterrows(), df_16.iterrows(), df_32.iterrows(), df_64.iterrows())):
        if any([row_8[1]["jailbroken"], row_16[1]["jailbroken"], row_32[1]["jailbroken"], row_64[1]["jailbroken"]]):
            ...
            plot_outputs(prompt, [row_4[1], row_8[1], row_16[1], row_32[1], row_64[1]], all_r, all_jailbroken, filename)
